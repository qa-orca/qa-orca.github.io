"""
Good/Better/Best scenario runner
Runs only GBB scenarios with age 15 cutoff testing
"""
...
# Define scenarios to test (6 total)
SCENARIOS_TO_RUN = [
    # Accident-only scenarios (3) - Age 15, no GBB testing
    ("gbb_accident_dog_15", None),  # 1. Dog aged 15 - accident-only
    ("gbb_accident_cat_15", None),  # 2. Cat aged 15 - accident-only
    ("gbb_accident_mixed_age", None),  # 3. Dog young + Cat 15 - mixed (dog has GBB, cat has AO)
...
def run_gbb_batch():
    """Run Good/Better/Best scenarios"""

    print("=" * 60)
    print("GOOD/BETTER/BEST SCENARIO RUNNER")
    print("=" * 60)
    print(f"\nTotal scenarios to run: {len(SCENARIOS_TO_RUN)}")
    print("\nThis will run:")
    print("  - 3 Accident-Only scenarios (age 15)")
    print("  - 3 Good/Better/Best scenarios (age under 15)")
    print("\nAccident-Only scenarios:")
    print("  1. Dog aged 15 - accident-only coverage")
    print("  2. Cat aged 15 - accident-only coverage")
    print("  3. Dog young + Cat 15 - mixed (dog GBB, cat AO)")
    print("\nGBB scenarios:")
    ...
    output_folder = f"output_gbb_{timestamp}"
    os.makedirs(output_folder, exist_ok=True)
    print(f"\n📁 Output folder: {output_folder}")
    print(f"   All scenarios will save to this folder")

    input("\nPress Enter to start GBB test run...")
